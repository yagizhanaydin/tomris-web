...
import { rejectJoinRequest } from "@/lib/chat/groups-public";
...
export async function POST(
  ...
  try {
    await rejectJoinRequest(conversationId, auth.uid, targetUid);
    return NextResponse.json({ success: true });
  } catch (err) {
    const code = err instanceof Error ? err.message : "error";
    if (code === "not_leader") {
      return NextResponse.json({ error: "Yalnızca grup lideri reddedebilir." }, { status: 403 });
    }
